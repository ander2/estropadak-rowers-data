...
def calculate_alta_baja(aurtengoa, aurrekoa, team):
    ...
    for a in aurtengo_arraunlariak:
        izena = a.name
        badago = [b.name for b in aurreko_arraunlariak if compare_rower_name(a.name, b.name)]
        if len(badago) == 0:
            altak.append(izena)

    for a in aurreko_arraunlariak:
        badago = [b.name for b in aurtengo_arraunlariak if compare_rower_name(a.name, b.name)]
        if len(badago) == 0:
            bajak.append(a.name)
